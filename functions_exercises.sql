...
# SELECT concat (first_name, last_name) as Name from employees WHERE last_name like 'E%' and '%e' ;
...
#number 6
select datediff(NOW(), hire_date) as Days_on_Job, emp_no, concat_ws(' ',first_name,last_name) as Employee
from employees where month(birth_date) = 12 and day(birth_date) = 25
                 and (year(hire_date) between 1990 and 1999)
order by Days_on_Job desc limit 20;
